...
def positional_encode(x, input_dim=80):

    encoder = PositionalEncoding(d_model=input_dim)
    output = encoder(x.repeat_interleave(input_dim, -1).unsqueeze(0)).squeeze()
    return output
...
def compute_pitch(y):
    ...
    pitch = np.array([[max_mag_idx[frame]] for frame in range(len(pitch))]) # (frame, pitch)
