...
class PM_QUANTIZED(Pipeline):
    ...
    def train_step(self, idx, batch):
        ...
        model_output_list, commit_loss_list = self.model(mel_input, pitch)
        ...
        t_l = mel_loss_list[0] + mel_loss_list[1] * 0.2 + mel_loss_list[2] * 0.04 + commit_loss_list[0] + commit_loss_list[1] * 0.2 + commit_loss_list[2] * 0.04 + commit_loss_list[3] * 0.2
        t_l.backward()
    ...
    def inference(self, source_path: str):
        ...
        with torch.no_grad():
            for i in range(len(mel_segments)):
                ...
                model_output_list, commit_loss_list = model(model_input, pitch_input)
...
if __name__ == '__main__':
    ...
    pipeline = PM_QUANTIZED(
        model = model,
        weight_dir = "models/pitch_quantized",
        log_dir = "logger/pitch_quantized"
    )
