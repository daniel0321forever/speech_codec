...
from model import Codec, CodecQ
from dataset import NSCDataset, NSCDataset_QPM
...
class PM_Q(Pipeline):
    ...
    def train_step(self, idx, batch):
        ...
        mel_input = batch[0].to(self.device) # take only the mel-cepstrum
        pitch = positional_encoding(batch[1]).to(self.device) # (frames=80, PE=64)
        mag = positional_encoding(batch[2]).to(self.device) #(frames=80, PE=64)
        ...
        if idx % 4000 == 0:
            print('mel loss: {:.4f} {:.4f} {:.4f}; commit loss: {:.4f} {:.4f} {:.4f}'.format(
                    mel_loss_list[0].item(), mel_loss_list[1].item(), mel_loss_list[2].item(),
                    commit_loss_list[0].item(), commit_loss_list[1].item(), commit_loss_list[2].item()), commit_loss_list[3].item())
            
        
        t_l = mel_loss_list[0] + mel_loss_list[1] * 0.2 + mel_loss_list[2] * 0.04 + commit_loss_list[0] + commit_loss_list[1] * 0.2 + commit_loss_list[2] * 0.04 + commit_loss_list[3] * 8
        t_l.backward()
    ...
    def inference(self, source_path: str):
        ...
        pitch, mag = compute_pitch(y)
        max_mag_idx = np.argmax(mag, axis=1)    
        pitch = np.array([[max_mag_idx[frame]] for frame in range(len(pitch))]) # (frames, 1)
        mag = np.array([[mag[frame][max_mag_idx[frame]]] for frame in range(len(mag))]) # (frames, 1)
...
if __name__ == '__main__':
    ...
    pipeline = PM_Q(
        model = model,
        weight_dir = "models/pitch_mag_quantized_q",
        log_dir = "logger/pitch_mag_quantized_q",
    )

    pipeline.train(
        train_set_path = os.path.join(root_dir, "dataset/train_spc_next.pkl"),
        test_set_path = os.path.join(root_dir, "dataset/test_spc_next.pkl"),
        batch_size=16,
    )

    pipeline.test("/media/daniel0321/LargeFiles/datasets/VCTK/raw_data/test/wav48_silence_trimmed/")

    pipeline.generate(audio_path="raw_data_sample/test/wav48_silence_trimmed/p343/p343_004_mic1.flac", save_dir="performance/pm_linear_Q")